class TaskManager {
    ...
    showTaskPopup(element) {
        ...
        // *** FIX: The loop was empty. It now generates the HTML for each checkbox. ***
    ...
    /**
     * Applies the selected tasks from the popup to the element.
    ...
    applyTasks(element) {
        ...
        const oldTasks = element.dataset.assignedTasks ? JSON.parse(element.dataset.assignedTasks) : [];

        // Find existing task container for this element, if any
        let taskContainer = document.querySelector(`.task-container[data-linked-element-id="${element.dataset.id}"]`);

        if (selectedTasks.length > 0) {
            element.dataset.assignedTasks = JSON.stringify(selectedTasks);
            
            if (taskContainer) {
                // Update existing task container
                taskContainer.dataset.assignedTasks = JSON.stringify(selectedTasks);
                if(taskContainer.querySelector('.task-container-header')) {
                   taskContainer.querySelector('.task-container-header').innerText = selectedTasks.join(', ');
                }
                
                // Also update the color of the progress ring based on the new first task
                const progressCircle = taskContainer.querySelector('.progress-ring__fg');
                if (progressCircle) {
                    const firstTaskColor = this.taskColors[selectedTasks[0]] || '#3498db';
                    progressCircle.style.stroke = firstTaskColor;
                }

            } else {
                // Create a new movable task display
                const elementRect = element.getBoundingClientRect();
                // Assumes existence of a 'screenToVirtual' function
                if (typeof screenToVirtual === 'function' && typeof createMovableTaskDisplay === 'function') {
                    const initialPosition = screenToVirtual(elementRect.left, elementRect.top - 80); // Position it above
                    createMovableTaskDisplay(initialPosition, element.dataset.id, selectedTasks);
                }
            }
        } else {
            // If no tasks are selected, remove the attribute and the task container
        ...
        this.updateTaskDisplay(element); // Update the image color
        this.hideTaskPopup();
        if (typeof saveDrawingState === 'function') saveDrawingState(); // Persist the changes
    }
